...
use Dlparker1005\MozartTest\Vendor\Stripe\StripeObject;
...
abstract class Util
{
    ...
    /**
     * Converts a response from the Dlparker1005\MozartTest\Vendor\Stripe API to the corresponding PHP object.
     *
     * @param array $resp the response from the Dlparker1005\MozartTest\Vendor\Stripe API
     * @param array $opts
     *
    ...
    {
        $types = \Dlparker1005\MozartTest\Vendor\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \Dlparker1005\MozartTest\Vendor\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \Dlparker1005\MozartTest\Vendor\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
