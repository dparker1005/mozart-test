...
trait Request
{
     ...
     *
     * @throws \Dlparker1005\MozartTest\Vendor\Stripe\Exception\InvalidArgumentException if $params exists and is not an array
     */
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            $message = 'You must pass an array as the first argument to Dlparker1005\MozartTest\Vendor\Stripe API '
               . 'method calls.  (HINT: an example call to create a charge '
               . "would be: \"Dlparker1005\MozartTest\Vendor\Stripe\\Charge::create(['amount' => 100, "
               . "'currency' => 'usd', 'source' => 'tok_1234'])\")";

            throw new \Dlparker1005\MozartTest\Vendor\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    ...
    {
        $opts = \Dlparker1005\MozartTest\Vendor\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \Dlparker1005\MozartTest\Vendor\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
